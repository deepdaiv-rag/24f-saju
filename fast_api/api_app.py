...
async def process_rag(conversation_history: List[dict]):
    try:
        # SajuRequest 생성
        input = SajuRequest(
            conversation_history=conversation_history, type=SajuRequestType.EXTRACT
        )
        ...
        return {
            ...
            "extraction_result": result.extraction_result.dict()
...
async def process_rag(
    conversation_history: List[dict], saju_info: dict, extraction_result: dict
):
    try:
        # SajuRequest 생성
        input = SajuRequest(
