...
class WebConnector(BaseConnector):
    ...
    async def get_document(self, input: ConnectorInput) -> List[ConnectorOutput]:
        ...
        naver_post, google_post = await asyncio.gather(
            self.search_naver_post(input.query),
            self.search_google_post(input.query)
        )
    ...
    async def search_google_post(self, query: str) -> List[str]:
        ...
        try:
            # 구글 검색 결과에서 링크 추출
            page = await self.browser.new_page()
            await page.goto(f"https://www.google.co.kr/search?q={query}", wait_until="domcontentloaded")
