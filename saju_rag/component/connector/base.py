...
class BaseConnector(ConnectorPort):
    ...
    def find_similar_chunks(self, query: str, text_list: List[str], top_n: int = 5, chunk_size: int = 1024) -> List[Tuple[str, float]]:
        """쿼리와 가장 유사한 청크를 찾는 함수."""
        # 각 텍스트를 청크로 나누기
        chunks = [self.chunk_text(text, chunk_size=chunk_size) for text in text_list]  # 각 문서별로 청크 나누기
        ...
        # 청크 임베딩 계산
        chunk_embeddings = np.array([self.compute_embedding(chunk) for chunk in chunks]).reshape(-1, self.embedding_model.config.hidden_size)
