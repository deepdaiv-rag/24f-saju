...
class AnswerWithExternalInfoUsecase:
    def __init__(
        ...
        connector_provider_port: ConnectorProviderPort,
    ):
        ...
        # 커넥터를 제공하는 모듈
        self.connector_provider_port = connector_provider_port
    ...
    async def execute(self, request: SajuRequest) -> str:
        ...
        prompt = (
            self.job_selector_prompt
            + self.connector_provider_port.get_all_connector_info()
        )
        job_selector_result = self.select_connector_port.select_connector(
            request, prompt
        )
        connector: ConnectorPort = self.connector_provider_port.select_connector(
            job_selector_result.get("connector")
        )
        query: str = job_selector_result.get("query")
        connector_input = ConnectorInput(
            query=query,
            extraction_result=str(request.extraction_result),
            saju_info=str(request.saju_info),
        ...
        response = (
            await self.answer_with_connector_info_port.answer_with_connector_info(
                prompt, request
            )
        )
        return response
